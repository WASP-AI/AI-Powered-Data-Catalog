...
if files:
    ...
    for file in files:
        ...
        for predefined_question in questions_list:
            ...
            # Merge the session state answers with all_answers
    for file_name, file_answers in st.session_state["questions_and_answers"].items():
        if file_name in all_answers:
            all_answers[file_name].update(file_answers)
    ...
    st.write(
        answer_df.style.set_table_styles(
            [
    ...
    st.markdown(f'<p style="color:#00ff00">Total Score: {total_score}/{max_total_score}</p>', unsafe_allow_html=True)
